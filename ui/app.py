...
status = "analyzing" if st.session_state.get("analyzing") else "ready"
st.markdown(
    f"""
...
if st.session_state.result:
    ...
    with col2:
        ...
        <div class="panel">
            ...
            <p style="color: var(--text); line-height: 1.5;">{result['summary']}</p>
        </div>
        """,
            unsafe_allow_html=True,
        )
        ...
        if st.button("Hear report"):
            with st.spinner("Generating voice..."):
                speak_res = requests.post(
                    f"{API_URL}/speak", params={"text": result["summary"]}
                )
                if speak_res.status_code == 200:
                    audio_path = speak_res.json()["audio_path"]
    ...
    if audio:
        ...
        with st.spinner("Transcribing..."):
            with open("temp_recording.wav", "rb") as f:
                res = requests.post(f"{API_URL}/transcribe", files={"file": f})
            if res.status_code == 200:
                question = res.json()["text"]
                st.markdown(
                    f'<div class="console-response">heard: {question}</div>',
                    unsafe_allow_html=True,
                )
    ...
    if st.button("Ask", key="ask_button") and question:
        with st.spinner("Thinking..."):
            ...
            if res.status_code == 200:
                ...
                st.markdown(
                    f'<div class="console-response">{answer}</div>',
                    unsafe_allow_html=True,
                )

                speak_res = requests.post(f"{API_URL}/speak", params={"text": answer})
                if speak_res.status_code == 200:
                    audio_path = speak_res.json()["audio_path"]
                    with open(audio_path, "rb") as f:
                        st.audio(f.read(), format="audio/mp3")
